...
def printAsserts(f):
    ...
    printAutoInd(f, '')
    # # 定义Barrier的Asset 目前只加入了三种
    # printAutoInd(f, '% Barrier Assets')
    # printAutoInd(f,
    #              'HighwayFence = roadrunner.hdmap.RelativeAssetPath(AssetPath="Assets/Extrusions/HighwayFence01.rrext");')
    # printAutoInd(f, 'rrMap.BarrierTypes(1) = roadrunner.hdmap.BarrierType(ID="HighwayFence",ExtrusionPath=HighwayFence);')
    # printAutoInd(f,
    ...
    # printAutoInd(f, 'rrMap.BarrierTypes(3) = roadrunner.hdmap.BarrierType(ID="FShapeBarrier",ExtrusionPath=FShapeBarrier);')
